...
import {
    ...
    SkipForward, SkipBack, AlertTriangle, Wifi, MonitorPlay
} from "lucide-react";
...
function HlsPlayer({
    ...
    useEffect(() => {
        ...
        const proxied = `/api/proxy/stream?url=${encodeURIComponent(src)}`;
        if (srcType === "hls") {
            if (Hls.isSupported()) {
    ...
    return (
        ...
        >
            <video ref={videoRef} className="w-full h-full object-contain" playsInline data-testid="video-player" />
            ...
            {activeIntro && (
                <button onClick={e => { e.stopPropagation(); skipTo(intro!.end); }}
            ...
            {activeOutro && (
                <button onClick={e => { e.stopPropagation(); skipTo(outro!.end); }}
            ...
            <div className={`absolute inset-0 flex flex-col justify-end transition-opacity duration-300 ${showControls ? "opacity-100" : "opacity-0"}`}
                ...
                <div className="px-3 pb-1.5" onClick={e => e.stopPropagation()}>
                    {hoverTime !== null && (
                        <div className="absolute bottom-14 text-[11px] font-mono text-white bg-black/80 px-1.5 py-0.5 rounded pointer-events-none z-20 -translate-x-1/2"
                            style={{ left: hoverX + 12 }}>{formatTime(hoverTime)}</div>
                    )}
                    <div ref={seekbarRef} className="relative cursor-pointer group/seek py-2 -my-2"
                        onMouseMove={onSeekbarMouseMove} onMouseDown={onSeekbarMouseDown}
                        onMouseUp={onSeekbarMouseUp} onMouseLeave={onSeekbarMouseLeave} data-testid="seekbar">
                        <div className="relative h-1 group-hover/seek:h-[5px] transition-all duration-100 rounded-full" style={{ background: "rgba(255,255,255,0.18)" }}>
                            <div className="absolute left-0 top-0 h-full rounded-full" style={{ width: `${buffPct}%`, background: "rgba(255,255,255,0.22)" }} />
                            <div className="absolute left-0 top-0 h-full rounded-full" style={{ width: `${pct}%`, background: "#a78bfa" }} />
                            {intro && duration > 0 && (
                                <div className="absolute rounded-[2px] pointer-events-none"
                                    style={{ left: `${(intro.start / duration) * 100}%`, width: `${Math.max(((intro.end - intro.start) / duration) * 100, 0.5)}%`, top: "-3px", bottom: "-3px", background: "#f59e0b", zIndex: 10 }}
                                    title={`Intro: ${formatTime(intro.start)}–${formatTime(intro.end)}`} />
                            )}
                            {outro && duration > 0 && (
                                <div className="absolute rounded-[2px] pointer-events-none"
                                    style={{ left: `${(outro.start / duration) * 100}%`, width: `${Math.max(((outro.end - outro.start) / duration) * 100, 0.5)}%`, top: "-3px", bottom: "-3px", background: "#f59e0b", zIndex: 10 }}
                ...
                <div className="flex items-center gap-2 px-3 pb-3" onClick={e => e.stopPropagation()}>
                    ...
                    <button onClick={rewind5} className="flex items-center gap-0.5 text-white hover:text-white/80 transition-colors" title="Rewind 5 seconds (←)" data-testid="button-rewind-5">
                        <SkipBack className="w-4 h-4 fill-white" /><span className="text-[10px] font-bold tabular-nums">5</span>
                    </button>
                    <button onClick={forward10} className="flex items-center gap-0.5 text-white hover:text-white/80 transition-colors" title="Skip forward 10 seconds (→)" data-testid="button-forward-10">
                        <span className="text-[10px] font-bold tabular-nums">10</span><SkipForward className="w-4 h-4 fill-white" />
                    </button>
                    <span className="text-white text-xs font-mono shrink-0 tabular-nums">{formatTime(currentTime)} / {formatTime(duration)}</span>
                    ...
                    <button onClick={e => { e.stopPropagation(); toggleFullscreen(); }} className="text-white hover:text-white/80 transition-colors" data-testid="button-fullscreen">
...
function IframePlayer({ src, sourceName }: { src: string; sourceName: string }) {
    const containerRef = useRef<HTMLDivElement>(null);
    const [fullscreen, setFullscreen] = useState(false);
    ...
    return (
        <div ref={containerRef} className="relative bg-black rounded-xl overflow-hidden aspect-video" data-testid="iframe-player-container">
            <iframe
                src={src}
            ...
            <div className="absolute top-2 right-2 flex items-center gap-2 z-10">
                <span className="text-[10px] font-mono text-white/60 bg-black/50 px-2 py-0.5 rounded backdrop-blur-sm">{sourceName}</span>
                <button onClick={toggleFullscreen} className="text-white/70 hover:text-white bg-black/50 rounded p-1 backdrop-blur-sm transition-colors" data-testid="button-iframe-fullscreen">
...
function EpisodeList({ total, watched, selected, onSelect }: {
    ...
    useEffect(() => { selRef.current?.scrollIntoView({ block: "nearest", behavior: "smooth" }); }, [selected]);
        ...
        <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                    {Array.from({ length: ranges }, (_, i) => i + 1).map(r => (
                        <button key={r} onClick={() => setRange(r)}
                            className={`px-2.5 py-0.5 rounded text-xs font-medium transition-all ${r === range ? "bg-primary text-primary-foreground" : "bg-muted/50 text-muted-foreground hover:bg-muted/80 border border-border/30"}`}>
            ...
            <div className="max-h-52 overflow-y-auto rounded-xl border border-border/30 bg-muted/10 divide-y divide-border/20">
                {Array.from({ length: end - start + 1 }, (_, i) => start + i).map(ep => {
                    ...
                    return (
                        <button key={ep} ref={isSel ? selRef : undefined} onClick={() => onSelect(ep)}
                            data-testid={`button-episode-${ep}`}
                            className={`w-full flex items-center gap-3 px-4 py-2.5 text-left text-sm transition-all hover:bg-muted/40 ${isSel ? "bg-primary/15 border-l-2 border-primary text-primary font-semibold" : isWatched ? "text-muted-foreground/70" : "text-foreground"}`}>
                            <span className={`text-xs font-mono w-8 shrink-0 ${isSel ? "text-primary" : "text-muted-foreground/60"}`}>{ep}</span>
                            <span className="flex-1 truncate">Episode {ep}</span>
                            {isWatched && !isSel && <span className="w-1.5 h-1.5 rounded-full bg-emerald-500/60 shrink-0" />}
                            {isSel && (
...
// ── Stream Panel (AllAnime-powered) ──────────────────────────────────────────

const LOADING_LOGS = [
    "Finding episode sources…",
...
function StreamPanel({
    showId, animeTitle, episode, watched, totalEps, lang,
    malId, anilistId,
    onEpisodeChange, onChangeSource,
}: {
    showId: string; animeTitle: string; episode: number; watched: number; totalEps: number;
    lang: "sub" | "dub";
    malId?: number | null; anilistId?: number | null;
    ...
    const load = useCallback(async () => {
        ...
        setError("");
        setLogIdx(0);
        const timer = setInterval(() => setLogIdx(i => (i + 1) % LOADING_LOGS.length), 2000);
        try {
            ...
            const srcs: StreamSource[] = json.sources || [];
            if (srcs.length === 0) throw new Error("No video sources found for this episode");
            setSources(srcs);
            setState("ready");
        } catch (err: any) {
            clearInterval(timer);
    ...
    return (
        <div className="space-y-4">
            ...
            {state === "loading" && (
                <div className="aspect-video rounded-xl border border-border/40 bg-black flex flex-col items-center justify-center gap-5">
                    ...
                    <div className="text-center space-y-1.5">
                        ...
                        <p className="text-xs text-muted-foreground font-mono">{LOADING_LOGS[logIdx]}</p>
                    </div>
            ...
            {state === "ready" && current && (
                <>
                    {(current.type === "hls" || current.type === "mp4") ? (
                        <HlsPlayer
                            src={current.url}
                            srcType={current.type}
                            malId={malId}
                            anilistId={anilistId}
                            episode={episode}
                            onError={() => {
                                if (sourceIdx + 1 < sources.length) {
                                    ...
                                    setError("All sources failed. Try a different server.");
                                    setState("error");
                                }
                            }}
                        />
                    ) : (
                        <IframePlayer src={current.url} sourceName={current.sourceName} />
                    )}

                    {/* Server switcher */}
                    {sources.length > 1 && (
                        <div className="flex flex-wrap gap-1.5 items-center">
                            <span className="text-[11px] text-muted-foreground/60 shrink-0">Server:</span>
                            {sources.map((s, i) => (
                                <button key={i} onClick={() => setSourceIdx(i)}
                                    data-testid={`button-source-${i}`}
                                    className={`px-2.5 py-0.5 rounded text-[11px] font-medium transition-all border ${i === sourceIdx ? "bg-primary text-primary-foreground border-primary" : "bg-muted/30 text-muted-foreground border-border/40 hover:bg-muted/60"}`}>
                                    {s.sourceName}
                                </button>
                            ))}
                        </div>
            ...
            <p className="text-[11px] text-muted-foreground/40 flex items-center gap-1">
                ...
                Streaming via AllAnime · Content belongs to respective rights holders.
            </p>
        </div>
    );
}
...
export default function Watch({ animeList }: { animeList: Anime[] }) {
    ...
    const [results, setResults] = useState<AllAnimeResult[]>([]);
    const [searching, setSearching] = useState(false);
    const [searched, setSearched] = useState(false);
    const [selected, setSelected] = useState<AllAnimeResult | null>(null);
    ...
    const doSearch = useCallback(async (q: string) => {
        ...
        setSearched(false);
        try {
            const res = await fetch(`/api/allanime/search?q=${encodeURIComponent(q)}`);
            const json = await res.json();
    ...
    const selectAnime = (result: AllAnimeResult) => {
        setSelected(result);
        setSelectedEp(1);
        setLang("sub");
    };
    ...
    const listAnime = useMemo(() => {
        ...
        const t = selected.name.toLowerCase();
        return animeList.find(a =>
            (selected.malId && a.malId === selected.malId) ||
    ...
    const totalEps = lang === "dub"
        ? (selected?.dubEpisodes || selected?.subEpisodes || listAnime?.totalEpisodes || 24)
        : (selected?.subEpisodes || listAnime?.totalEpisodes || 24);
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                        ...
                        <div className="flex-1 min-w-0">
                            <h3 className="font-bold text-base truncate">{selected.name}</h3>
                            <p className="text-[11px] text-muted-foreground">
                                {selected.subEpisodes > 0 ? `${selected.subEpisodes} episodes` : ""}
                                {selected.dubEpisodes > 0 ? " · Dub available" : ""}
                            </p>
                        </div>
                        ...
                        <div className="flex items-center shrink-0 rounded-lg border border-border/50 overflow-hidden">
                            ...
                            <button
                                ...
                                disabled={!selected.dubEpisodes}
                                className={`px-3 py-1.5 text-xs font-semibold transition-all disabled:opacity-40 disabled:cursor-not-allowed ${lang === "dub" ? "bg-primary text-white" : "text-muted-foreground hover:text-foreground hover:bg-muted/40"}`}>
                                DUB
                            </button>
                        </div>
                    </div>
                    ...
                    <StreamPanel
                        key={`${selected.id}-${lang}-${selectedEp}`}
                        showId={selected.id}
                        animeTitle={selected.name}
                        episode={selectedEp}
                        watched={listAnime?.episodesWatched || 0}
                        totalEps={totalEps}
                        lang={lang}
                        malId={listAnime?.malId || selected.malId}
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
                            {results.map(r => {
                                const listEntry = animeList.find(a =>
                                    (r.malId && a.malId === r.malId) ||
                                    a.title.toLowerCase() === r.name.toLowerCase()
                                );
                                return (
                                    <button key={r.id} onClick={() => selectAnime(r)}
                                        ...
                                        <div className="aspect-[3/4] overflow-hidden bg-muted/30 relative">
                                            {r.thumbnail
                                                ...
                                                : <div className="w-full h-full flex items-center justify-center"><MonitorPlay className="w-8 h-8 opacity-20" /></div>
                                            }
                                        ...
                                        <div className="p-2">
                                            ...
                                            <p className="text-[10px] text-muted-foreground mt-0.5">
                                                {r.subEpisodes > 0 ? `${r.subEpisodes} eps` : ""}
                                                {r.dubEpisodes > 0 ? " · DUB" : ""}
                                            </p>
                                        </div>
                                    </button>
